...
@DATASET_REGISTRY.register()
class YUVRecurrentDataset(data.Dataset):
    """Modified based on REDS dataset for training recurrent networks.
    ...
    def __init__(self, opt):
        super(YUVRecurrentDataset, self).__init__()
        self.opt = opt
        self.gt_root, self.lq_root = Path(opt['dataroot_gt']), Path(opt['dataroot_lq'])
        self.num_frame = opt['num_frame']

        file_list = os.listdir(self.gt_root)
        self.keys = []
        self.num_record = {}
        for file_name in file_list:
            pic_list = os.listdir(os.path.join(self.lq_root, file_name))
            self.num_record[file_name] = len(pic_list)
            for pic_name in pic_list:
                self.keys.extend([file_name+'/'+pic_name])
    ...
    def __getitem__(self, index):
        ...
        total_frame = self.num_record[clip_name]

        # determine the neighboring frames
        interval = random.choice(self.interval_list)

        # ensure not exceeding the borders
        start_frame_idx = int(frame_name.split('.')[0].split('_')[-1])
        frame_name_head = frame_name.split('.')[0].split('_')[0]

        if start_frame_idx > total_frame - self.num_frame:
            start_frame_idx = random.randint(0, total_frame - self.num_frame)
        end_frame_idx = start_frame_idx + self.num_frame
        ...
        for neighbor in neighbor_list:
            if self.is_lmdb:
                ...
                img_lq_path = os.path.join(self.lq_root, clip_name, frame_name_head+'_'+str(neighbor)+'.png')
                img_gt_path = os.path.join(self.gt_root, clip_name, frame_name_head+'_'+str(neighbor)+'.png')
...
class YUVDataset(data.Dataset):
    """Modified based on REDS dataset for training single frame networks.
    ...
    def __getitem__(self, index):
        ...
        for neighbor in neighbor_list:
            ...
            img_gt_list.append(img_gt_path)

        img_gt_path = img_gt_list[self.num_frame//2]
        # get GT
        img_bytes = self.file_client.get(img_gt_path, 'gt')
        img_gt = imfrombytes(img_bytes, float32=True)
