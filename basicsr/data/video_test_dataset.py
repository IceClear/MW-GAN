...
@DATASET_REGISTRY.register()
class YUVTestDataset(data.Dataset):
    """Video test dataset.
    Generally, it supports testing dataset with following structures:
    ...
    def __init__(self, opt):
        super(YUVTestDataset, self).__init__()
        self.opt = opt
        self.cache_data = opt['cache_data']
        self.frame_limited = opt['frame_limited']
        self.gt_root, self.lq_root = opt['dataroot_gt'], opt['dataroot_lq']
        self.data_info = {'lq_path': [], 'gt_path': [], 'folder': [], 'idx': [], 'border': []}
        # file client (io backend)
        self.file_client = None
        ...
        for subfolder_lq, subfolder_gt in zip(subfolders_lq, subfolders_gt):
            ...
            img_paths_lq = sorted(img_paths_lq,key=lambda x:(int(x.split('.')[-2].split('_')[-1])))
            img_paths_gt = sorted(img_paths_gt,key=lambda x:(int(x.split('.')[-2].split('_')[-1])))

            frame_limited = min(self.frame_limited, len(img_paths_lq))
            img_paths_lq = img_paths_lq[:frame_limited]
            img_paths_gt = img_paths_gt[:frame_limited]
...
@DATASET_REGISTRY.register()
class YUVRecurrentTestDataset(YUVTestDataset):
    """Video test dataset for recurrent architectures, which takes LR video
    frames as input and output corresponding HR video frames.
    ...
    def __init__(self, opt):
        super(YUVRecurrentTestDataset, self).__init__(opt)
        # Find unique folder strings
        self.folders = sorted(list(set(self.data_info['folder'])))
